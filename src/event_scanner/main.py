...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    @app.get("/legacy-scanner", response_class=HTMLResponse, include_in_schema=False)
        ...
        if not rows:
            ...
            for item in live_items:
                ...
                try:
                    ...
                    continue
            rows = service.repository.recent(50)
        body = "<nav style='margin-bottom:16px'><a href=\"/?lang=en\">Event Feed</a> · <a href=\"/cat-rank\">Market Intelligence</a> · <a href=\"/volume-fire\">Volume Fire</a> · <a href=\"/methodology\">Data Sources & Method</a></nav>" + "".join(
    ...
    def modern_dashboard(lang: str = 'en'):
        rows = service.repository.recent(50)
        cards = ''.join(f"<article class='card {escape(row['level'])}'><div class='tag'>{escape(row['level'].upper())} · {row['score']}/100</div><h3><a href='{escape(row['source_url'])}' target='_blank' rel='noreferrer'>{escape(row['title'])}</a></h3><div class='contract'>{escape(row['symbol'] or 'UNMAPPED')}</div><p>{escape(', '.join(row['reasons']))}</p><small>{escape(row['source_name'] or 'Source')} · {escape(row['occurred_at'] or row['detected_at'])}</small></article>" for row in rows) or "<article class='card'><div class='tag'>LIVE MONITORING</div><h3>Scanning trusted market sources</h3><p>Verified risk events, mapped contracts, source evidence, and market confirmation appear here as they are detected.</p><small>RSS · Official project communications · Binance futures</small></article>"
        return f"""<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Crypto Scanner</title><style>body{{margin:0;background:#0e131b;color:#f5f7fb;font:16px Arial,sans-serif}}a{{color:inherit;text-decoration:none}}.top{{padding:22px 32px;display:flex;gap:28px;align-items:center;border-bottom:1px solid #252d39}}.brand{{color:#438cff;font-size:28px;font-weight:800}}.nav{{display:flex;gap:24px;color:#bdc4d0;flex:1}}.nav a:first-child{{color:#55a1ff;border-bottom:3px solid #55a1ff;padding-bottom:20px}}.lang{{background:#202733;padding:9px;border-radius:12px}}.ticker{{padding:18px 32px;background:#121923;color:#53a1ff;word-spacing:18px;white-space:nowrap;overflow:hidden}}.notice{{margin:18px 32px;padding:22px 28px;border-radius:15px;background:linear-gradient(90deg,#eb1616,#ff6900);font-size:22px;font-weight:700}}main{{padding:48px 32px;max-width:1400px;margin:auto}}.eyebrow{{color:#438cff;font-weight:bold}}h1{{font-size:48px;margin:18px 0 8px}}.sub{{color:#b7c0ce;font-size:20px}}.panel{{margin-top:36px;background:#171d26;border:1px solid #2a3441;border-radius:14px;padding:28px}}.panelhead{{display:flex;justify-content:space-between;align-items:center}}.badge{{background:#242d39;padding:10px 14px;border-radius:10px;color:#c8d1de}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(290px,1fr));gap:16px;margin-top:22px}}.card{{background:#151b24;border:1px solid #2a3441;border-radius:12px;padding:20px}}.tag{{color:#55a1ff;font-size:13px;font-weight:bold}}.high .tag{{color:#ff6767}}.medium .tag{{color:#ffbe5b}}h3{{margin:12px 0;font-size:20px}}.contract{{font-size:24px;font-weight:bold}}p,small{{color:#b8c2d0;line-height:1.5}}</style></head><body><header class='top'><div class='brand'>CryptoScanner</div><nav class='nav'><a href='/'>Events</a><a href='/cat-rank'>Markets</a><a href='/volume-fire'>Volume Fire</a><a href='/methodology'>Research</a></nav><div class='lang'><a href='/?lang=ko'>KO</a> · <a href='/?lang=en'>EN</a></div></header><div class='ticker'>BTCUSDT · Live Binance Futures · ETHUSDT · Structural-risk monitoring · RSS & official sources</div><div class='notice'>● Real-time Crypto Risk Monitor — Source-linked events and market validation →</div><main><div class='eyebrow'>EVENTS</div><h1>Event Trading Dashboard</h1><div class='sub'>Source, mapped perpetual contract, risk score, evidence, and detected time.</div><section class='panel'><div class='panelhead'><div><h2>Live Structural-Risk Events</h2><p>PRD event ingestion · classification · scoring · Binance symbol mapping</p></div><div class='badge'>Auto refresh 30 sec</div></div><div class='grid'>{cards}</div></section></main></body></html>"""

    @app.get('/methodology', response_class=HTMLResponse, include_in_schema=False)
    def methodology():
        return """<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Data Sources & Method</title><style>body{margin:0;background:#0b1020;color:#e7eaf0;font:15px system-ui;padding:28px;max-width:1000px}a{color:#93c5fd}section{background:#161d31;border:1px solid #26314d;border-radius:10px;padding:18px;margin:14px 0}h2{margin-top:0}li{margin:8px 0;color:#c7d2e3}</style></head><body><nav><a href='/'>Event Feed</a> · <a href='/cat-rank'>Market Intelligence</a> · <a href='/volume-fire'>Volume Fire</a></nav><h1>Data Sources & Methodology</h1><section><h2>Event data</h2><ul><li>Official X/project communications, trusted crypto news RSS, and normalized webhook sources.</li><li>Every event retains source URL, source name, occurred time, detection time, title, and raw risk evidence.</li></ul></section><section><h2>Asset and market validation</h2><ul><li>Named assets are mapped to active Binance USDT perpetual contracts.</li><li>Binance data provides 24-hour price change, quote volume, liquidity eligibility, candle-based volume signals, and contract rankings.</li><li>On-chain confirmation can be supplied by a verified provider; missing confirmation never upgrades a signal by itself.</li></ul></section><section><h2>Signal policy</h2><ul><li>Risk score (0–100): structural-event severity, source trust, asset mapping, market confirmation, on-chain confirmation, and liquidity.</li><li>High: trusted structural risk plus market or on-chain confirmation. Medium: risk detected but incomplete confirmation. Hold: duplicate, rumor, irrelevant, or unmapped.</li><li>All signals are decision support only, never automated order execution or investment advice.</li></ul></section></body></html>"""
